...
def wizard_form_header():
    ...
    step_cols2[2].button('Interest Rate',on_click=set_form_step,args=['Jump',6],type=interest_rate_type)        
    step_cols2[3].button('Number of Loans',on_click=set_form_step,args=['Jump',7],type=number_loans_type)      
    step_cols2[4].button('Maximum Delay',on_click=set_form_step,args=['Jump',8],type=max_delay_type)

    step_cols3[1].button('Number of Delayed Payments',on_click=set_form_step,args=['Jump',9],type=num_delayed_payments_type)
    step_cols3[2].button('Credit Mix',on_click=set_form_step,args=['Jump',10],type=credit_mix_type)        
    step_cols3[3].button('Credit History Age',on_click=set_form_step,args=['Jump',11],type=credit_history_type)      
    step_cols3[4].button('Monthly Balance',on_click=set_form_step,args=['Jump',12],type=monthly_balance_type)
...
def wizard_form_body():
    ...
    if st.session_state['current_step'] == 6:
        st.markdown('\n')
        st.markdown('\n')                          
        st.number_input('Type the last interest rate that you paid:', key = '_last_interest_rate', value = st.session_state.last_interest_rate, on_change = update_last_interest_rate)                             

    ###### Step 7: Number of loans ######
    if st.session_state['current_step'] == 7:
        st.markdown('\n')
        st.markdown('\n')                          
        st.number_input('Type how many loans have you had in the past:', key = '_Num_of_Loan', value = st.session_state.Num_of_Loan, on_change = update_Num_of_Loan)                

    ###### Step 8: Maximum month delay ######
    if st.session_state['current_step'] == 8:
        st.markdown('\n')
        st.markdown('\n')                             
        st.number_input('Type the maximum month delay you have had from payment due date:', key = '_month_delay', value = st.session_state.month_delay, on_change = update_month_delay)                

    ###### Step 9: Maximum payments delay ######
    if st.session_state['current_step'] == 9:
        st.markdown('\n')
        st.markdown('\n')                            
        st.number_input('Type the maximum payments delay you have had:', key = '_payments_delay', value = st.session_state.payments_delay, on_change = update_payments_delay)                
    ...
    if st.session_state['current_step'] == 11:
        st.markdown('\n')
        st.markdown('\n')                               
        st.number_input('Type the number of years you have had a credit history:', key = '_Credit_History_Age', value = st.session_state.Credit_History_Age, on_change = update_Credit_History_Age)     

    ###### Step 12: Monthly Balance ######
    if st.session_state['current_step'] == 11:
        st.markdown('\n')
        st.markdown('\n')                            
        Monthly_Balance = st.number_input('Type your last monthly balance:', key = '_Monthly_Balance', value = st.session_state.Monthly_Balance, on_change = update_Monthly_Balance)  
