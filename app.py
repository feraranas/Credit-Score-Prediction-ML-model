...
def wizard_form_header():
    ...
    step_cols1[1].button('Month', on_click=set_form_step, args=[
                         'Jump', 1], type=month_type, disabled=st.session_state.month_disabled)
    step_cols1[2].button('Age', on_click=set_form_step, args=[
                         'Jump', 2], type=age_type, disabled=st.session_state.age_disabled)
    step_cols1[3].button('Income', on_click=set_form_step, args=[
                         'Jump', 3], type=income_type, disabled=st.session_state.income_disabled)
    step_cols1[4].button('Bank Accounts', on_click=set_form_step, args=[
                         'Jump', 4], type=bank_accounts_type, disabled=st.session_state.num_bank_accounts_disabled)

    step_cols2[1].button('Credit Cards', on_click=set_form_step, args=[
                         'Jump', 5], type=credit_cards_type, disabled=st.session_state.num_credit_card_disabled)
    step_cols2[2].button('Interest Rate', on_click=set_form_step, args=[
                         'Jump', 6], type=interest_rate_type, disabled=st.session_state.last_interest_rate_disabled)
    step_cols2[3].button('Number of Loans', on_click=set_form_step, args=[
                         'Jump', 7], type=number_loans_type, disabled=st.session_state.Num_of_Loan_disabled)
    step_cols2[4].button('Maximum Delay', on_click=set_form_step, args=[
                         'Jump', 8], type=max_delay_type, disabled=st.session_state.month_delay_disabled)

    step_cols3[1].button('Number of Delayed Payments', on_click=set_form_step, args=[
                         'Jump', 9], type=num_delayed_payments_type, disabled=st.session_state.payments_delay_disabled)
    step_cols3[2].button('Credit Mix', on_click=set_form_step, args=[
                         'Jump', 10], type=credit_mix_type, disabled=st.session_state.credit_mix_disabled)
    step_cols3[3].button('Credit History Age', on_click=set_form_step, args=[
                         'Jump', 11], type=credit_history_type, disabled=st.session_state.Credit_History_Age_disabled)
    step_cols3[4].button('Monthly Balance', on_click=set_form_step, args=[
                         'Jump', 12], type=monthly_balance_type, disabled=st.session_state.Monthly_Balance_disabled)
...
def wizard_form_body():
    ...
    #    age = st.number_input('Type your age', value = age, on_change = update_age)
        st.number_input('Type your age', key='_age',
                        value=st.session_state.age, on_change=update_age, min_value=0)
    ...
    if st.session_state['current_step'] == 3 and not st.session_state.income_disabled:
        ...
        st.number_input('Type your monthly income', key='_income',
                        value=st.session_state.income, on_change=update_income, min_value=0)
    ...
    if st.session_state['current_step'] == 4 and not st.session_state.num_bank_accounts_disabled:
        ...
        st.number_input('Type the number of distinct bank accounts that you have:', key='_num_bank_accounts',
                        value=st.session_state.num_bank_accounts, on_change=update_bank_accounts, min_value=0)
    ...
    if st.session_state['current_step'] == 5 and not st.session_state.num_credit_card_disabled:
        ...
        st.number_input('Type the number of distinct credit cards that you have:', key='_num_credit_card',
                        value=st.session_state.num_credit_card, on_change=update_credit_card, min_value=0)
    ...
    if st.session_state['current_step'] == 6 and not st.session_state.last_interest_rate_disabled:
        ...
        st.number_input('Type the last interest rate that you paid:', key='_last_interest_rate',
                        value=st.session_state.last_interest_rate, on_change=update_last_interest_rate, min_value=0)
    ...
    if st.session_state['current_step'] == 7 and not st.session_state.Num_of_Loan_disabled:
        ...
        st.number_input('Type how many loans have you had in the past:', key='_Num_of_Loan',
                        value=st.session_state.Num_of_Loan, on_change=update_Num_of_Loan, min_value=0)
    ...
    if st.session_state['current_step'] == 8 and not st.session_state.month_delay_disabled:
        ...
        st.markdown('\n')
        st.markdown('\n')
        st.number_input('Type the maximum month delay you have had from payment due date:', key='_month_delay',
                        value=st.session_state.month_delay, on_change=update_month_delay, min_value=0)
    ...
    if st.session_state['current_step'] == 9 and not st.session_state.payments_delay_disabled:
        ...
        st.number_input('Type the maximum payments delay you have had:', key='_payments_delay',
                        value=st.session_state.payments_delay, on_change=update_payments_delay, min_value=0)
    ...
    if st.session_state['current_step'] == 11 and not st.session_state.Credit_History_Age_disabled:
        ...
        st.number_input('Type the number of years you have had a credit history:', key='_Credit_History_Age',
                        value=st.session_state.Credit_History_Age, on_change=update_Credit_History_Age, min_value=0)
    ...
    if st.session_state['current_step'] == 12 and not st.session_state.Monthly_Balance_disabled:
        ...
        Monthly_Balance = st.number_input('Type your last monthly balance:', key='_Monthly_Balance',
                                          value=st.session_state.Monthly_Balance, on_change=update_Monthly_Balance, min_value=0)
    ...
    with form_footer_container.container():
        ...
        disable_next_button = True if st.session_state[
            'current_step'] == 12 or st.session_state.next_disabled else False
